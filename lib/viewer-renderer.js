...
`;
  ...
  const bootstrap = {
    ...
    // Race mode (v0.33.155+)
    // Debug / diagnostics (v0.33.194+). Both default false.
    // debugOverlayEnabled: activates the sync debug overlay without needing ?debug=1 in the URL.
    // playerStatsEnabled: shows the drift/calibration readout in the player bar status line.
    debugOverlayEnabled: cfg.debug_overlay_enabled === 1,
    playerStatsEnabled: cfg.player_stats_enabled === 1,
...
.race-instructions p { margin: 0; }
  ...
  <script src="/rf-compat.js?v=77"></script>
  `;
  ...
  // ---- Demo banner (v0.31.0+) ----
  // Demo banner — only injected when demoMode is explicitly enabled in
  // config.js. On production installs this block is skipped entirely,
  // keeping view-source clean and saving the bandwidth of injecting
  // ~3KB of CSS/HTML/JS onto every page load.
  const _appConfig = require('../lib/config-loader');
  if (_appConfig.demoMode) {
